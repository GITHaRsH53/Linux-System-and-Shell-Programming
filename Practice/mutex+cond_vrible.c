...
// threads execution in order using condition variable(thread synchronization)
...
void * threadA(void *p) {
    ...
    doneFlag = 1;
    pthread_cond_signal(&myConVar); // chanegs done now can execute the changes in the process
...
void * threadB(void *p) {
    ...
    pthread_mutex_lock(&my_mutex); // my_mutex locks
    if (doneFlag == 0) // if thread A has not worked on it
    pthread_cond_wait(&myConVar, &my_mutex); // then wait until changes reflect in myConVar. my_mutex unlocks
